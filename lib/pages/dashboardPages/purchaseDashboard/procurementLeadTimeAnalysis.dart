...
class ProcurementLeadTimeAnalysis extends StatefulWidget {
  ...
  @override
  State<ProcurementLeadTimeAnalysis> createState() =>
      _ProcurementLeadTimeAnalysisState();
}
...
List<ProcurementLeadTimeList> leadTimeList = [];
List<ProcurementLeadTimeList> leadTimeListUniqueGrnNo = [];
List<Users> usersList = [];

bool chartDataLoaded = false;

SupplierWiseLeadTimeList supplierList = SupplierWiseLeadTimeList(
  supplierLeadTime: [],
);
ItemWiseLeadTimeList itemList = ItemWiseLeadTimeList(itemLeadTime: []);
...
class ProcurementLeadTimeProvider with ChangeNotifier {
  List<ProcurementLeadTimeList> _salesList = [];
  List<ProcurementLeadTimeList> get salesList => _salesList;
  void updatePurchaseList(List<ProcurementLeadTimeList> newSalesList) {
    _salesList = newSalesList;
    notifyListeners();
  }
}

class _ProcurementLeadTimeAnalysisState
    extends State<ProcurementLeadTimeAnalysis> {
  ...
  SideTitles get _bottomTitlesSupplierWiseLeadtimeAnalysis => SideTitles(
    ...
    getTitlesWidget: (value, meta) {
      String text = '';
      List<SupplierWiseLeadTimeData> mData = supplierList.supplierLeadTime;
  ...
  SideTitles get _bottomTitlesItemWiseLeadtimeAnalysis => SideTitles(
    ...
    getTitlesWidget: (value, meta) {
      String text = '';
      List<ItemWiseLeadTimeData> mData = itemList.itemLeadTime;
  ...
  List<BarChartGroupData> _supplierWiseLeadtimeAnalysisChartData(
    List<SupplierWiseLeadTimeData> data,
  ) {
    return data
        .map(
  ...
  List<BarChartGroupData> _itemWiseLeadtimeAnalysisChartData(
    List<ItemWiseLeadTimeData> data,
  ) {
    return data
        .map(
          (chartData) => BarChartGroupData(
  ...
  Future<void> _loadProductionCompletedReportAnalysis(
    ...
    int limit = 10000;
    int fetchedCount = 0;
    List<ProcurementLeadTimeList> salesList = [];
    ...
    try {
      do {
        ...
        const apiUrl =
            '${ApiHelper.baseUrl}CRMProcurementAnalysisReport'
            '';
        ...
        if (response.statusCode == 200) {
          ...
          if (responseJson["responseData"].toString().isNotEmpty) {
            List<ProcurementLeadTimeList> newSalesList =
                (responseJson['responseData'] as List)
                    .map((item) => ProcurementLeadTimeList.fromJson(item))
                    .toList();
      ...
      setState(() {
        leadTimeList = salesList;
        context.read<ProcurementLeadTimeProvider>().updatePurchaseList(
          salesList,
        );
        ...
        leadTimeList = salesList.toList();
        Set<String> seenGrnNos = {};

        for (var item in leadTimeList) {
          if (seenGrnNos.add(item.grnNo)) {
            leadTimeListUniqueGrnNo.add(item);
          }
        }
      });
    } catch (e) {
  ...
  List<ProcurementLeadTimeList> filterProcurementList(
    List<ProcurementLeadTimeList> procurementList, {
    String? itemCode,
    String? supplierCode,
  }) {
    List<ProcurementLeadTimeList> filteredProcurementList = [];
    for (var procurement in procurementList) {
      if ((itemCode == null ||
              itemCode.isEmpty ||
              procurement.productName == itemCode) &&
          (supplierCode == null ||
              supplierCode.isEmpty ||
              procurement.supplierName == supplierCode)) {
        filteredProcurementList.add(procurement);
      }
    }
    return filteredProcurementList;
  }
  ...
  Future<void> loadDataWithFilter(String itemCode, String supplierCode) async {
    clearVariablesForFilter();
    LoadDates();
    await _loadSupplierWiseLeadTime(itemCode, supplierCode);
    await _loadItemWiseLeadTime(itemCode, supplierCode);
    chartDataLoaded = true;
  }
  ...
  void clearVariables() {
    setState(() {
      chartDataLoaded = false;
      supplierList = SupplierWiseLeadTimeList(supplierLeadTime: []);
      itemList = ItemWiseLeadTimeList(itemLeadTime: []);
      touchedItemCode = "";
      touchedSupplierCode = "";
    });
  }
  ...
  Future<void> _loadSupplierWiseLeadTime(
    String itemCode,
    String supplierCode,
  ) async {
    var leadList = filterProcurementList(
      leadTimeListUniqueGrnNo.cast<ProcurementLeadTimeList>().toList(),
      itemCode: itemCode,
    ...
    List<SupplierWiseLeadTimeData> supplierData = [];
    Set<String> processed = {};
    Map<String, int> supplierCountMap = {};

    for (var itemGroup in leadList) {
      if (!processed.contains(itemGroup.supplierName)) {
        supplierName = itemGroup.supplierName;

        supplierCountMap[supplierName] = 0;
        totalLeadTime = 0;

        for (var target in leadList.where(
          (prdelement) => prdelement.supplierName == supplierName,
        )) {
          String aStr = target.pOtoDDLeadtime.replaceAll(RegExp(r'[^0-9]'), '');
        ...
        processed.add(itemGroup.supplierName);
      }
    }

    supplierData.sort((a, b) => b.leadTime.compareTo(a.leadTime));

    supplierList = SupplierWiseLeadTimeList(supplierLeadTime: supplierData);
  }

  Future<void> _loadItemWiseLeadTime(
    String itemCode,
    String supplierCode,
  ) async {
    var leadList = filterProcurementList(
      leadTimeList.cast<ProcurementLeadTimeList>().toList(),
      itemCode: itemCode,
      supplierCode: supplierCode,
    );
    String itemName = "";
    double totalLeadTime = 0.00;
    double avgLeadTime = 0.00;
    List<ItemWiseLeadTimeData> itemData = [];
    Set<String> processed = {};
    ...
    for (var itemGroup in leadList) {
      if (!processed.contains(itemGroup.productName)) {
        ...
        itemData.add(
          ItemWiseLeadTimeData(itemName: itemName, leadTime: avgLeadTime),
        );
    ...
    itemList = ItemWiseLeadTimeList(itemLeadTime: itemData);
  }
  ...
  Future<void> loadData(String selectedUser) async {
    ...
    await _loadProductionCompletedReportAnalysis(selectedUser, userLevel);
  ...
  Future<void> _dateFilterTarget(
    ...
    setState(() {
      ...
      leadTimeList = leadTimeList.where((target) {
                  ...
                  setState(() {
                    if (flTouchEvent is FlTapUpEvent) {
                      touchedSupplierCode = touchedSupplierCode == ""
                          ? supplierList
                                .supplierLeadTime[barTouchResponse.spot!.spot.x
                                    .toInt()]
                                .supplierName
                          : "";
                      selectedChart = barTouchResponse.spot!.spot.x;
                      showDrillDownChart = true;
                      loadDataWithFilter(touchedItemCode, touchedSupplierCode);
                    }
                  });
                }
              },
                ...
                getTooltipItem: (groupData, grpIndex, rodData, rodIndex) {
                  return BarTooltipItem(
                    ...
                    children: <TextSpan>[
                      TextSpan(
                        text:
                            "Lead Time : ${(supplierList.supplierLeadTime[grpIndex].leadTime).toStringAsFixed(2)}",
                  ...
                  setState(() {
                    if (flTouchEvent is FlTapUpEvent) {
                      touchedItemCode = touchedItemCode == ""
                          ? itemList
                                .itemLeadTime[barTouchResponse.spot!.spot.x
                                    .toInt()]
                                .itemName
                          : "";
                      selectedChart = barTouchResponse.spot!.spot.x;
                ...
                getTooltipItem: (groupData, grpIndex, rodData, rodIndex) {
                  return BarTooltipItem(
                    ...
                    children: <TextSpan>[
                      TextSpan(
                        text:
                            "Lead Time : ${(itemList.itemLeadTime[grpIndex].leadTime).toStringAsFixed(2)}",
                        style: const TextStyle(
                          color: Colors.black, //widget.touchedBarColor,
                          fontSize: 12,
                          fontWeight: FontWeight.w500,

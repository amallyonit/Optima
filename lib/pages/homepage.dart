...
class HomePageState extends State<HomePage> {
  ...
  List<ScheduleWithParticipants> _groupSchedules(
    ...
    return baseById.entries.map((e) {
      final pid = e.key;
      return ScheduleWithParticipants(
        e.value,
        byId[pid] ?? <String>[],
      );
    }).toList();
  ...
  Future<void> loadData() async {
    ...
    String? filterDate = DateFormat('dd/MM/yyyy')
        .format(DataManager.readSelectedDate()!)
        .toString();

    if (userRoleCode == "R1" || userRoleCode == "R2") {
      await _selectCheckinDetails(userId, userJwtToken, userMailID, filterDate);
      ...
      await _selectLeadsDetails(userId, userJwtToken, userMailID, filterDate);
      await _selectLeadActivity(userId, userJwtToken, userMailID, filterDate);
      await _selectScheduledLeadActivity(
          userId, userJwtToken, userMailID, filterDate);
    }
  }

  Future<void> _selectLeadsDetails(String userId, String userJwtToken,
      String userMailID, String filterDate) async {
    final data = {
      'UserID': userId,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseJson['Status'] == true &&
          ...
          if (data.isNotEmpty) {
            ...
            if (data.length > 1 && data[1] is List) {
              ...
              setState(() {
                context
                    .read<LeadContactHomePageProvider>()
            ...
            const snackBar = SnackBar(
              content: Text('Leads are not found.'),
            );
            ScaffoldMessenger.of(context).showSnackBar(snackBar);
          }
        } else {
        ...
        const snackBar = SnackBar(
          content: Text('Leads data is not available'),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
      }
    } catch (e) {
        ...
        if (responseJson['Status'] == true &&
          ...
          if (data.isNotEmpty) {
            if (data[0] is List) {
              ...
              setState(() {
                context
                    .read<LeadActivityHomePageProvider>()
  ...
  Future<void> _selectScheduledLeadActivity(String userId, String userJwtToken,
      String userMailID, String filterDate) async {
    final data = {
      'UserID': userId,
      'UserJwtToken': userJwtToken,
  ...
  Future<void> _selectCheckinDetails(String userId, String userJwtToken,
      String userMailID, String filterDate) async {
    List<CheckinDetails> newList = [];
    setState(() {
      newList = [];
      context
          .read<CheckinDetailsHomePageProvider>()
          .updateCheckinDetails(newList);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        const snackBar = SnackBar(
          content: Text('Checkin details not found.'),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
      }
    } catch (e) {
  ...
  Future<void> _loadmonthlyscheduler(
    ...
    String? filterDate = DateFormat('yyyy-MM-dd')
        .format(DataManager.readSelectedDate()!)
        .toString();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (status && responseJson["Data"].toString().isNotEmpty) {
          List<dynamic> data = responseJson['Data'];
          if (data.isNotEmpty) {
            List<MonthlySchedule> newMonthlySchedule =
                data.map((item) => MonthlySchedule.fromJson(item)).toList();
            if (mounted) {
              setState(() {
                context
                ...
                isRejected =
                    List.filled(monthlyScheduleListNotifications.length, false);
                reasons =
                    List.filled(monthlyScheduleListNotifications.length, '');
              });
            }
          }
        } else {
  ...
  Future<void> _updatemonthlyschedulestatus(
      int scheduleId, String scheduleStatus, String scheduleRemarks) async {
    final prefs = await SharedPreferences.getInstance();
    final userId = prefs.getString('userId') ?? '';
    final userJwtToken = prefs.getString('userJwtToken') ?? '';
    final userMailID = prefs.getString('userMailID') ?? '';
  ...
  Widget customerData() {
    LeadActivityHomePageProvider leadActivityProvider =
        context.watch<LeadActivityHomePageProvider>();
    ...
    CheckinDetailsHomePageProvider checkinDetailsProvider =
        context.watch<CheckinDetailsHomePageProvider>();
    checkinDetails = checkinDetailsProvider.checkinDetails;

    LeadContactHomePageProvider leadContactProvider =
        context.watch<LeadContactHomePageProvider>();
                                ...
                                child: scheduledLeadActivity
                                    ...
                                    ? Column(
                                        ...
                                        crossAxisAlignment:
                                            CrossAxisAlignment.start,
                                        children: <Widget>[
                                          for (var item in scheduledLeadActivity
                                              .where((element) =>
                                                  element.leadType == "A"))
                                            Column(
                                              children: [
                ...
                Visibility(
                  ...
                  child: Column(
                    children: [
                      Padding(
                        padding:
                            const EdgeInsetsDirectional.fromSTEB(20, 0, 16, 4),
                        child: Row(
                          mainAxisSize: MainAxisSize.max,
                          mainAxisAlignment: MainAxisAlignment.start,
                          children: [
                                                                    ...
                                                                    Expanded(
                                                                      child:
                                                                        ...
                                                                        children: [
                                                                          const Padding(
                                                                            padding:
                                                                                EdgeInsets.only(top: 2.0, right: 6),
                                                                            child:
                                                                                Icon(
                                                                              Icons.people_outline,
                                                                              size: 18,
                                                                          ...
                                                                          Text(
                                                                            item.leadParticipantUserName.split(",").first.toString(),
                                                                            style:
                                                                                const TextStyle(
                                                                              fontFamily: "Roboto",
                                                                              fontSize: 14,
                                                                          ...
                                                                          Visibility(
                                                                            visible:
                                                                                item.leadParticipantUserName.split(",").length - 1 != 0,
                                                                            child:
                                                                                GestureDetector(
                                                                              onTap: () {
                                                                                showDialog(
                                                                    ...
                                                                    Expanded(
                                                                      child:
                                                                        ...
                                                                        children: [
                                                                          ...
                                                                          if (leadContacts.any((element) =>
                                                                              element.leadContactMasterId ==
                                                                              item.leadActivityMasterId))
                                                                            Text(
                                                                              leadContacts.where((element) => element.leadContactMasterId == item.leadActivityMasterId).first.leadContactName,
                                                                              style: const TextStyle(
                                                                                fontFamily: "Roboto",
                                                                                fontSize: 14,
                                                                                fontWeight: FontWeight.w300,
                                                                          ...
                                                                          Visibility(
                                                                            visible:
                                                                                leadContacts.where((element) => element.leadContactMasterId == item.leadActivityMasterId).length - 1 != 0,
                                                                            child:
                                                                                GestureDetector(
                                                                              onTap: () {
                                                                                String parts = "";
                                                                                String parts2;
                                                                                leadContacts.where((element) => element.leadContactMasterId == item.leadActivityMasterId).forEach((element) {
                                                                    ...
                                                                    Row(
                                                                      mainAxisSize:
                                                                          MainAxisSize
                                                                              .min,
                                                                      children: [
                                                                        for (var item in leadContacts.where((element) =>
                                                                            element.leadContactMasterId ==
                                                                            item.leadActivityMasterId))
                                                                          buildStack(),
                                                                      ],
                ...
                Visibility(
                  ...
                  child: Column(
                    children: [
                      ...
                      Padding(
                        padding:
                            const EdgeInsetsDirectional.fromSTEB(20, 5, 20, 12),
                        child: Container(
                          width: double.infinity,
                          height: 240,
                          decoration: BoxDecoration(
                                                              ...
                                                              children: [
                                                                ...
                                                                Expanded(
                                                                  child:
                                                                    ...
                                                                    subtitle: isRejected[index] &&
                                                                            reasons[index].isNotEmpty
                                                                        ? Padding(
                                                                            padding:
                                                                      ...
                                                                      actions: [
                                                                        TextButton(
                                                                          onPressed:
                                                                              () async {
                                                                            setState(() {
                                                                              isRejected[index] = true;
                                                                              reasons[index] = _reasonController.text.trim();
                                                                            });
                                                                            await _updatemonthlyschedulestatus(
                                                                                monthlyScheduleListNotifications[index].scheduleID,
                                                                                'R',
                                                                                reasons[index]);
